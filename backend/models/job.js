...
class Job {
  ...
  static async findAll(data, username) {
    ...
    if (data.results_per_page){
      ...
      limitQuery = ` LIMIT ${data.results_per_page} OFFSET ${data.results_per_page * Math.min(page-1, 0)}`
    }
    // Finalize query and return results

    let finalQuery = baseQuery + whereExpressions.join(" AND ") + limitQuery;
  ...
  static async unapply(job_id, username) {
